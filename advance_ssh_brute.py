...
# Queue to hold username-password combos
q = queue.Queue()
...
# Password generator (on-the-fly brute-force)
...
def worker(host):
    while not q.empty():
        ...
        if is_ssh_open(host, username, password):
            ...
            q.queue.clear()  # Stop other threads
            break
        q.task_done()
...
def main():
    ...
    parser.add_argument("--min-length", type=int, default=1, help="Minimum length of generated passwords")
    parser.add_argument("--max-length", type=int, default=4, help="Maximum length of generated passwords")
    parser.add_argument("--chars", type=str, default=string.ascii_lowercase + string.digits, help="Characters to use for password generation")
